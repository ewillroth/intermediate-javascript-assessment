...
String.prototype.grammarPolice = function(){
	...
	let answer = arr.map(word => {
		...
		let secondhalf = word.slice(1,word.length)
		let answer = firsthalf.toUpperCase() + secondhalf.toLowerCase()
		console.log(answer)
		return answer
	})
...
const promiseCatcher = (promise) => {
	promise.then(response => { theAnswer = response}).catch(err=> console.log(err))
}
